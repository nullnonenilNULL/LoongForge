...
LOAD=${LOAD:-"/mnt/cluster/LoongForge/moonshotai/Kimi-K2.6-entp1dtp8pp8ep16etp1"}
SAVE=${SAVE:-"/mnt/cluster/LoongForge/moonshotai/Kimi-K2.6-hf-bf16"}
...
echo "===== Kimi K2.6 full M-Core -> BF16 HuggingFace conversion ($(date '+%F %T')) ====="
echo ">> LOG:                $LOG"
echo ">> LOAD:               $LOAD"
echo ">> SAVE:               $SAVE"
...
if [[ ! -f "$OFFICIAL_CONFIG_FILE" ]]; then
    echo "Missing official Kimi K2.6 config file: $OFFICIAL_CONFIG_FILE" >&2
    echo "Set OFFICIAL_HF_PATH or OFFICIAL_CONFIG_FILE to a local Kimi K2.6 HuggingFace metadata directory." >&2
    exit 1
fi
...
# quantization metadata are skipped so the BF16 checkpoint remains unquantized.
if [[ "$NODE_RANK" == "0" ]]; then
...
def mark_bf16_unquantized(config):
...
for src in official_hf_path.iterdir():
    ...
    if src.name == "config.json":
        with src.open("r", encoding="utf-8") as f:
            config = json.load(f)
        config = mark_bf16_unquantized(config)
...
cat <<EOF
Saved BF16 HuggingFace checkpoint to: $SAVE

You can use this checkpoint as NVIDIA ModelOpt input for NVFP4, for example:
  BF16_HF="$SAVE" \\
  bash "${LOONGFORGE_PATH}/examples/kimi_k2.x/kimi_k2.6/checkpoint_convert/nvfp4_ptq/convert_kimi_k2.6_hf_bf16_to_nvfp4_modelopt.sh"
EOF
