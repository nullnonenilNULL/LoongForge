...
class TransformerLayerSchedulePlan:
    ...
    * mtp.mtp_post_process contains output_layer, mtp loss operations, whereas
    ...
    def __init__(self, layer, event, chunk_state, comp_stream, comm_stream, extra_args={}):
        ...
        """
        from .fine_grained_callables import TransformerLayerState
    ...
    def _build_callable_nodes(self, event, comp_stream, comm_stream, extra_args):
        ...
        """
        from .fine_grained_callables import (
    ...
    def run(
        ...
        Two execution modes are supported, controlled by CUDA_DEVICE_MAX_CONNECTIONS:

        1) CUDA_DEVICE_MAX_CONNECTIONS = 1 (early_comm_launch=False):
        ...
        2) CUDA_DEVICE_MAX_CONNECTIONS = 32 (early_comm_launch = True):

        In this mode, CUDA allows more concurrent kernel launches.
        To better handle the operator sparsity at the beginning of the MLP backward phase,
        the launch of MLP backward is moved ahead of the dispatch forward,
        so as to improve overlap with forward computation and communication.
...
class TransformerModelChunkSchedulePlan(AbstractSchedulePlan):
    ...
    def __init__(
        ...
        *,
        ...
        """
        from .fine_grained_callables import PostProcessNode, PreProcessNode
        ...
        self._model_chunk_state.image_inputs = image_inputs
        self._model_chunk_state.video_inputs = video_inputs
        self._model_chunk_state.audio_inputs = audio_inputs
    ...
    def run(
        ...
        if b_num_layers > 0:
            with b_layer.get_fp8_context():
                b_grad = b_schedule_plan.get_layer(0).attn.backward(b_grad)
