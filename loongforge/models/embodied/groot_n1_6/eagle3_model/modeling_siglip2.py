"""Vendored Siglip2 vision model implementation for Eagle3 integration."""
...
# Import of external config classes removed - defined locally below for vendoring purposes
...
def _flash_attention_forward(
    ...
    **kwargs,
    ...
    # logger.info("Using flash_attn_varlen_func for training")
...
def flash_attention_forward_for_packing(
    ...
    **kwargs,
    ...
    """Run FlashAttention-2 forward for packed variable-length windows."""
...
# Local config class definitions for vendoring purposes
class Siglip2Config(PretrainedConfig):
    """Base Siglip2 config class for vendored implementation."""
    model_type = "siglip2"
...
class Siglip2TextConfig(PretrainedConfig):
    """Text config class for Siglip2 (dummy for vendored vision-only implementation)."""
...
class Siglip2VisionConfig(PretrainedConfig):
    ...
    >>> # Initializing a Siglip2VisionModel (with random weights)
    >>> # from the google/siglip2-base-patch16-naflex style configuration
...
class Siglip2VisionOutput(ModelOutput):
    ...
    Args:
        image_embeds (`torch.FloatTensor` of shape `(batch_size, output_dim)` *optional*,
            returned when model is initialized with `with_projection=True`):
...
def convert_images_to_patches(image: "torch.Tensor", patch_size: int) -> "torch.Tensor":
    """
    Convert 4D tensor image of shape
    (batch_size, num_channels, image_height, image_width) into 2D tensor of patches of shape
...
class Siglip2VisionEmbeddings(nn.Module):
    """Patch and positional embedding builder for Siglip2 vision inputs."""
    ...
    def get_spatial_shapes(self, bchw_list: list[torch.Tensor]) -> torch.Tensor:
        """Convert per-batch BCHW shapes into per-sample patch-space HW shapes."""
        hw_list = []
...
class Rope2DPosEmb(nn.Module):
    ...
    3. During the forward pass, pass the `freqs_cis` tensor to each
       attention layer, and call `apply` before each attention operation.
    ...
    Args:
        dim (int): usually the multi-head attention dimension, should be
            divisible by 4 (TODO: relax this constraint if needed)
...
def eager_attention_forward(
    ...
    **kwargs,
    ...
    """Compute eager attention outputs and attention weights."""
...
def apply_rope(
    ...
    Args: (The leading dimensions of all inputs should be the same)
        ...
        freqs_cis: tensor of shape (..., head_dim/2), dtype=torch.complex64.
            It contains precomputed cis(freqs) for each position in the 2D grid.
    Returns:
...
class Siglip2MLP(nn.Module):
    """Two-layer feed-forward block used inside Siglip2 encoder layers."""
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        """Apply the MLP projection, activation, and output projection."""
...
class Siglip2EncoderLayer(nn.Module):
    """Single transformer encoder block for Siglip2 vision features."""
...
class Siglip2VisionTransformer(nn.Module):
    """Siglip2 vision transformer stack with embeddings and encoder blocks."""
...
def variance_scaling_(tensor, scale=1.0, mode="fan_in", distribution="normal"):
    """Initialize tensor values using variance scaling strategy."""
...
def lecun_normal_(tensor):
    """Apply LeCun normal initialization via truncated-normal variance scaling."""
    variance_scaling_(tensor, mode="fan_in", distribution="truncated_normal")


def default_flax_embed_init(tensor):
    """Apply Flax-style default embedding initialization."""
...
class Siglip2PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        # Siglip2Model and Siglip2ForImageClassification branches removed.
        # Not used in this vendored vision-only implementation.
...
class Siglip2VisionModel(Siglip2PreTrainedModel):
    """Siglip2 vision-only model wrapper built on `Siglip2VisionTransformer`."""
    ...
    def get_input_embeddings(self) -> nn.Module:
        """Return patch embedding module used as vision input projection."""
